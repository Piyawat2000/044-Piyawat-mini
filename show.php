...
$sql = "SELECT * FROM tb_shoesproduct";
        ...
        <table class="table" id="data_table">
            <thead>
                <tr>
                    ...
                    <?php echo $_SESSION['role'] == $set_role ? "<th>Action</th>" : "" ?>
                </tr>
            </thead>
            <?php
            foreach ($products as $product) {
                ...
                echo $_SESSION['role'] == $set_role ? '<td>
                <form action="form_edit_shoe.php" method="POST" style="display:inline;">
                        <input type="hidden" name="id" value="'.$product['shoe_id'].'">
                        <!-- <input type="submit" name="delete" value="Delete" class="btn btn-danger btn-sm"> -->
                        <button type="submit" class="btn btn-warning"
                            data-user-id=" '.$product['shoe_id'].'">Edit</button>
                </form>
                    <form action="044_Delete.php" method="POST" style="display:inline;">
                        
                        <!-- <input type="submit" name="delete" value="Delete" class="btn btn-danger btn-sm"> -->
                        <button type="button" class="btn btn-danger delete-button"
                            data-shoe-id="'. $product['shoe_id'] .'">Delete</button>
                    </form>
